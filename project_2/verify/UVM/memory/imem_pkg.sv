package imem_pkg;
    ...
    class imem_driver extends uvm_driver #(imem_item);
        ...
        task run_phase(uvm_phase phase);
            forever begin
                ...
                @(posedge vif.clk);
                vif.addr <= req.addr;
                // Wait for memory access time ( RAM sync needs 1 cycle)

	       // Wait one more cycle (idld) to help Monitor separate transactions clearly
    ...
    class imem_monitor extends uvm_monitor;
		...
		// Detect address phase
                @(posedge vif.clk);
                // Only stare when rst = 0
		if (vif.rst == 0) begin
                item = imem_item::type_id::create("item");
             	#1;
	     	item.addr = vif.addr;
		// Wait for Data Phase (next cycle)
		@(posedge vif.clk);
		#1; 
		item.data = vif.instr;
                // Send to Scoreboard
    ...
    class imem_basic_test extends uvm_test;
        ...
        task run_phase(uvm_phase phase);
            ...
            repeat(1000) begin
                item = imem_item::type_id::create("item");
                item.addr = $urandom_range(0, 255) * 4;
		item.data = $urandom();
